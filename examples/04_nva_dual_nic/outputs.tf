...
output "router_inside_private_ip" {
  value = module.router_vm.vm_private_ips["inside"]
}

output "router_outside_private_ip" {
  value = module.router_vm.vm_private_ips["outside"]
}

output "router_nsg_ids" {
  value = {
    inside  = module.nsg_router_inside.id
    outside = module.nsg_router_outside.id
  }
}
